...
.DESCRIPTION
    This Script will Connect to Windows Computer
.NOTES
    File name: Test-Connection-SSH-Win.ps1
    VERSION: 1.0.0a
...
if($pingtest){
    ...
    $pingport = Test-NetConnection -ComputerName $VMNAME -Port $VM_SSHPORT -ErrorAction SilentlyContinue
    if($pingport){
        Write-Host( "The Port " +$VM_SSHPORT+" is open" )
        # $vmsess = New-PSSession -HostName $VMNAME -Port $$VM_SSHPORT -UserName $VM_USER -SSHTransport
        Exit 0
    }
    else {
        Write-Host( "The Port "+$VM_SSHPORT+" is not reacable")
        Exit 2
    }
}
else{
