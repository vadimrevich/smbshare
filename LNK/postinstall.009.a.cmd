...
set aMSG=%~dp0Msg.postinstall.009.a.Success.wsf
...
echo Check a Path ...
rem
set APATH=c:\.BIN\smbshare\NIT\NonInteractive\WSF
set APATH01=c:\.BIN\smbshare\NIT\NonInteractive\cmd
if not exist %APATH% echo "%APATH% not Exist. Check Windows Postinstall Procedures." && exit /b 1
if not exist %APATH01% echo "%APATH01% not Exist. Check Windows Postinstall Procedures." && exit /b 1
echo Check a File ...
rem
set AFILE=%APATH%\KMSDownload.wsf
set AFILE01=%APATH01%\Unzip-KMSTools.cmd
if not exist %AFILE% echo "%AFILE% not Exist. Check if Windows Antivirus Turned off." && exit /b 1
