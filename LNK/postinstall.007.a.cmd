...
set aMSG=%~dp0Msg.postinstall.007.a.Success.wsf
...
echo Check a Path ...
rem
set APATH=c:\.BIN\smbshare\NIT\DefenderDefeat\LNK
if not exist %APATH% echo "%APATH% not Exist. Check Windows Postinstall Procedures." && exit /b 1
echo Check a File ...
rem
set AFILE=%APATH%\Add_Exclusions.url
if not exist %AFILE% echo "%AFILE% not Exist." && exit /b 1
echo Execute a File...
start %AFILE%
