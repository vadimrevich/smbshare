...
set aMSG=%~dp0Msg.postinstall.005.Success.wsf
...
echo Check a Path ...
rem
set APATH=c:\.BIN\smbshare\NIT\Vagrant_Settings\Win
if not exist %APATH% echo "%APATH% not Exist. Check Windows Postinstall Procedures." && exit /b 1
echo Check a File ...
rem
set AFILE01=%APATH%\IEHArden_V5.bat
set AFILE02=%APATH%\WinRM2Unsafe.bat
set AFILE03=%APATH%\TurnOffServerManager.reg
set AFILE04=%APATH%\TurnOffShutdowsTracker.reg
if not exist %AFILE01% echo "%AFILE01% not Exist." && exit /b 1
if not exist %AFILE02% echo "%AFILE02% not Exist." && exit /b 1
if not exist %AFILE03% echo "%AFILE03% not Exist." && exit /b 1
if not exist %AFILE04% echo "%AFILE04% not Exist." && exit /b 1
echo Execute a File...
call %AFILE01%
call %AFILE02%
%pathCMD%\reg.exe import %AFILE03%
%pathCMD%\reg.exe import %AFILE04%
